# Demo Script — 8-Minute Arc (S5, archived)

> **Status:** Superseded by `demo-script.md` (v3+). Kept for reference only.
> **Reason for archival:** S5 opened with the portal. v3+ opens with research
> (decision log + hunt logs + live curl) to directly counter the recruiter's
> stated failure mode ("technical execution without research").
