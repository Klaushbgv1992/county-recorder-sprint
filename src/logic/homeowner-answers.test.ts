...
describe("computeHomeownerAnswers — POPHAM", () => {
  ...
  it("title-clean reflects open lifecycles tied to this parcel's instruments", () => {
    // lc-002 (root: 20210057847 DOT) + lc-015 (root: 20240100001 demo-only
    // federal tax lien) are both open and rooted in POPHAM's instrument list.
    expect(a.titleClean.clean).toBe(false);
    expect(a.titleClean.openCount).toBe(2);
    expect(a.titleClean.openLifecycleIds).toEqual(expect.arrayContaining(["lc-002", "lc-015"]));
  });
