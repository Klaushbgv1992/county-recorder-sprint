...
| 43 | County Heartbeat: citation-anchored, deterministic, live-feeling counter on landing page | `<CountyHeartbeat/>` mounts as first child of `<main>` in `LandingPage.tsx`. Counter total anchored to the Maricopa Recorder's public "About" page (~1M docs/yr ÷ 250 business days ≈ 4,000 docs/day); see `docs/data-provenance.md`. Intra-day pacing via `fractionOfDayFiled` — a piecewise-linear curve labeled DEMO-ONLY PACING MODEL, chosen for inspectability over realism. 24-hour sparkline renders the same curve as density bars (17 visible, 7 pre-dawn omitted with a 1px baseline). Ribbon copy interpolates `plant_lag_reference.lag_days_min`/`lag_days_max` from `pipeline-state.json`. Determinism invariant: `countAtTime(T)` is pure of `T`; rapid F5 returns the same number. `lagMin=0` render guard is thesis-preservation (plants equally current → hide rather than mislead). `now?: number` prop is the test/screenshot hatch; `?now=<ISO-with-tz>` parsed at the page level via `useNowOverrideFromSearchParams`. No `aria-live` (would spam at 1Hz). `PipelineBanner` and `RootLayout` untouched per parallel-execution discipline. | 2026-04-16 |
...
| 45 | 5 Seville Parcel 3 neighbors with pre-fetched recorder-API responses + tiered drawer model | 14 `/documents/{n}` responses cached for 5 Seville neighbors (BROTHERTON 304-78-406, SCHERF 304-78-338, KALANITHI 304-78-369, SOMMERFELD 304-78-408, ANGUS 304-78-367). Selection frozen in `scripts/lib/neighbor-instruments.ts`. Variable instrument count (2–3 per APN, not exactly 3): ANGUS (lot 45, POPHAM's literal next-door neighbor) has 2 instruments; uniform counts would have been cherry-picking. Doc-type mix: WAR DEED ×5, DEED TRST ×5, T FIN ST ×3, DISCLAIMR ×1, Q/CL DEED ×1. Tiered drawer: `Curated · full chain` (5) / `Recorder · cached` (5) / `Assessor · public GIS` (~8,570). Owner autocomplete silently suppressed on assessor tier. Overlays: encumbrance (default ON), anomaly (OFF), lastdeed (OFF). URL state unified in `useLandingUrlState` (`?q`, `?apn`, `?overlay`). Research budget: ~60 API calls across ~20 candidates at 1s spacing (15% hit rate for 3-instrument sets — SolarCity UCC termination pattern). |
| 46 | R-008 hero parcel locked: APN 304-77-566 (Lot 224 Shamrock 2A, 2778 E Courtney St Gilbert 85298) — trustee-succession messy chain | Robert F. + Linda L. Silva bought Lot 224 in 2015, financed with a $260K Wells Fargo HELOC, transferred into THE SILVA FAMILY REVOCABLE TRUST (est. 2007-07-26) in 2017, paid off the HELOC in 2023, and BOTH died on 2026-03-11 (consecutive death-certificate recording numbers 20260141249/20260141250). Nine days later, Successor Trustee Richard D. Moore conveyed the property out of the Silva Trust into his own MOORE REVOCABLE TRUST (est. 2021-08-16) via TRST DEED 20260162239. New parcel + 7 instruments + 2 lifecycles (lc-016 released HELOC pair, lc-017 succession) + 7 links curated. See `data/raw/R-008/hunt-log.md`. | 2026-04-18 |
| 47 | Known Gap #2 narrowed via R-008 | Public-API `documents/search` filters `lastNames=`/`firstNames=`/`middleNameIs=`/`businessNames=`/`pageSize=`/`pageNumber=` DO work; only `documentCode` and `docketBook`/`pageMap` are silently dropped. The R-008 handoff and Decision #40 framing of "name search broken at recorder.maricopa.gov" was caused by the wrong URL parameter name (`name=`) — the recorder's HTML name-search is backed by the public API with the parameter names listed above. Re-tested 2026-04-18: `documentCode=PR%20DEED` still returns 1947 PART REL records, confirming the documentCode-filter sub-finding from R-005. | 2026-04-18 |
| 48 | Same-business-name moat finding (R-008) | Two unrelated families both maintain a "Silva Family Revocable Trust" in Maricopa: Robert F. + Linda L. Silva est. 2007-07-26 (holds Lot 224 Shamrock 2A, the R-008 hero parcel), and David M. + Diana L. Silva est. 2016-06-22 (holds Lot 157 Ashland Ranch APN 309-20-157). A `businessNames=SILVA+FAMILY+REVOCABLE+TRUST` API query returns 12 instruments mingled across both trusts. Disambiguation requires the trust "established" date, which is in the deed body but not in API metadata. Same family as Decision #25 (same-name contamination) and Decision #21 (data-model gaps). | 2026-04-18 |
| 49 | "TRST DEED" doc-code is taxonomically overloaded (R-008) | Maricopa's "TRST DEED" code labels both (a) trustee's deeds upon foreclosure sale under A.R.S. § 33-811 (the Notice-of-Trustee-Sale → Sub Trustee → TRST DEED foreclosure sequence) and (b) trustee's deeds in trust-administration / successor-trustee transfers under A.R.S. § 33-404 (the R-008 hero instrument 20260162239). A name-indexed plant or examiner relying on doc-code alone cannot distinguish foreclosure from probate-equivalent succession without reading the body. Same family as Decision #21 (taxonomy mismatch) and Decision #25 (contamination). | 2026-04-18 |
| 50 | Custodial-asymmetry moat finding: indexed but body-restricted death certs (R-008) | Maricopa indexes death certificates so they're discoverable by name search via `documents/search?lastNames=...`, but the PDF body is restricted from public viewing on `legacy.recorder.maricopa.gov` (returns an "Information Blocked" HTML page in place of the PDF). The county itself, holding the source image, can read the body; a private title plant cannot. R-008 confirmed both Silva death certificates (20260141249 + 20260141250) are restricted-body. Same thesis family as Decision #16 (live-sync mention-only) and Decision #21 (county-only data). | 2026-04-18 | 2026-04-16 |
| 51 | Custodian query engine replaces static party-judgment-sweeps.json | Real-data engine over a captured Playwright + curl fixture; exposed through a `/custodian-query` matrix showpiece (5 parties × 2 live indexes × 2 approaches = 20 cells, auto-run on first visit) and the existing `PartyJudgmentSweep` parcel panel. 2 live indexes (MCR name index + MCSC civil judgments); 3 documented dead-ends (AZ DOR, IRS NFTL, USBC). POPHAM-only party scope; HOGUE remains blocked-contrast via engine `no_capture_available` state. AI judgments pre-baked in fixture; no runtime LLM. Capture serialized against other Playwright agents. MCSC scoutability + BRIAN MADISON collision pre-hunt both have documented fallbacks. | 2026-04-18 |
...
| R-008 | 4 | COMPLETE | Messy-parcel hunt — hero candidate locked: APN 304-77-566, Lot 224 Shamrock Phase 2A, 2778 E Courtney St Gilbert 85298. Trustee-succession chain: 2015 purchase → 2017 into Silva Family Revocable Trust (est. 2007-07-26) → 2023 HELOC release → 2026-03-11 simultaneous death of both trustees Robert+Linda Silva (consecutive death certs) → 2026-03-20 Successor Trustee Richard D. Moore conveys via TRST DEED to his own Moore Revocable Trust (est. 2021-08-16). Hunt log promoted to `data/raw/R-008/hunt-log.md` as third Beat-7 disintermediation receipt. See Decisions #46–#50. |
